...
flags = tf.app.flags
flags.DEFINE_string('input_path', './object_detection/faster_rcnn_resnet101_coco_2018_01_28/model.ckpt',
                    'path of pretrained_checkpoint')
flags.DEFINE_string('output_path', './object_detection/ckpts/model.ckpt', 'output checkpoint')
...
if __name__ == '__main__':
  reader = tf.train.NewCheckpointReader(FLAGS.input_path)
  ...
  for name, var_to_edit in zip(var_to_edit_names, vars_to_edit):
    if FLAGS.edit_method in ['spread', 'clone']:
      ...
      if num_input_channels % checkpoint_num_input_channels != 0:
        raise Exception('For spread edit method, num_input_channels must be divisible by num input channels of checkpoint!')
      num_clones = int(num_input_channels / checkpoint_num_input_channels)
      ...
      random_shape[2] = num_input_channels - 3
